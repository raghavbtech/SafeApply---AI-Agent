...
5. **Private Azure Blob Resume Storage**: Uploaded resumes (PDF, DOCX, TXT; 10MB limit) are validated using magic file signatures, stored in private Azure Blob Storage (with local filesystem fallback), and isolated per session. Resumes are downloaded via byte streaming; physical paths are never exposed. PDF resumes can also be previewed in the My Profile page through an authenticated, no-store byte response; DOCX resumes remain download-only.
