...
"""
SafeApply - Background Mailbox Worker
======================================
Standalone process: polls IMAP -> Cosmos -> auto-scan on a fixed interval,
...
from auto_scan import scan_all_unscanned, AUTO_QUARANTINE_ENABLED
from azure_db import DEFAULT_USER_ID

POLL_INTERVAL_SECONDS = int(os.getenv("SAFEAPPLY_POLL_INTERVAL", "300"))
FETCH_COUNT = int(os.getenv("SAFEAPPLY_SYNC_FETCH_COUNT", "20"))


def run_once(user_id: str = DEFAULT_USER_ID) -> None:
    if not is_mail_configured():
        print("[sync_worker] MAIL_USERNAME / MAIL_APP_PASSWORD not set; skipping.")
        return

    print(f"[sync_worker] {datetime.now(timezone.utc).isoformat()} syncing...")
    sync_result = sync_mailbox_to_db(max_messages=FETCH_COUNT, user_id=user_id)
    print(f"[sync_worker] sync result: {sync_result}")

    if not sync_result.get("ok"):
        return

    scan_results = scan_all_unscanned(user_id=user_id, allow_auto_quarantine=AUTO_QUARANTINE_ENABLED)
    quarantined = sum(1 for r in scan_results if r.get("quarantined"))
    print(f"[sync_worker] scanned {len(scan_results)}, quarantined {quarantined}")


def main() -> None:
    print(f"[sync_worker] starting. Poll interval: {POLL_INTERVAL_SECONDS}s")
    while True:
        try:
            run_once()
        except Exception as exc:  # noqa: BLE001
